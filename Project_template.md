...
1.  В папке `apps` выполните:
    ...
    ```
2.  Отправьте запрос `Create Sensor` из Postman-коллекции.
3.  Проверьте логи контейнеров `device-service` и `telemetry-service`, чтобы убедиться, что запрос прошел через всю цепочку.